import pysam
from design.MethylRead import MethylRead, mark_cpg_of_interest
...
def format_print(reference, read_type, read=None):
    ...
    if read_type == "reference":
        return ">" + reference.name  + "|number_of_cpgs=" + str(reference.cpg_number) + "|tissue=" + str(reference.tissue) + "|" + "reference", mark_cpg_of_interest(reference.reference, 124)

    elif read_type == "methylated":
        return ">" + reference.name  + "|number_of_cpgs=" + str(reference.cpg_number) + "|tissue=" + str(reference.tissue) + "|" + "fully_converted_methylated", mark_cpg_of_interest(reference.methylated, 124)

    elif read_type == "read":
        base = ">" + read.name  + "|read_number=" + str(read.read_number)

        if read.strand:
            return base + "| reverse_strand", align_read(reference, read)
        else:
            return base + "| forward_strand", align_read(reference, read)
...
def write_patterns(cpg, patterns_list, out):
    ...
    cpg_of_interest = cpg.cpg_locations.index(124)

    pattern_header = "> " + str(cpg.name) + "("
    cpg_number = 1
    for cpg in quantified_patterns:

        if cpg_number == cpg_of_interest:
            annotated_cpg = "*" + str(cpg_number) + "*"
        else:
            annotated_cpg = str(cpg_number)

        if cpg[0]+cpg[1] == 0:
            pattern_header += "cpg" + annotated_cpg + ": " + "NA, "
        else:
            prop_methylated = round(cpg[1]/(cpg[0]+cpg[1]), 3)
            pattern_header += "cpg" + annotated_cpg + ": " + str(prop_methylated) + ", "
        cpg_number += 1
